...
const S06 = Math.sqrt(0.6), S13 = 1 / Math.sqrt(3);
const G3 = [[-S06, 5 / 9], [0, 8 / 9], [S06, 5 / 9]];
const G2 = [[-S13, 1], [S13, 1]];
...
export const MATERIALS = {
  S235: { id: 'S235', name: 'S235 szerkezeti acél', E: 21000, nu: 0.3, sigmaY: 23.5, alpha: 1.2e-5, rho: 7850, Hp: 0, plastic: true, source: 'MSZ EN 10025-2', verified: true },
  S275: { id: 'S275', name: 'S275 szerkezeti acél', E: 21000, nu: 0.3, sigmaY: 27.5, alpha: 1.2e-5, rho: 7850, Hp: 0, plastic: true, source: 'MSZ EN 10025-2', verified: true },
  S355: { id: 'S355', name: 'S355 szerkezeti acél', E: 21000, nu: 0.3, sigmaY: 35.5, alpha: 1.2e-5, rho: 7850, Hp: 0, plastic: true, source: 'MSZ EN 10025-2', verified: true },
  S235H: { id: 'S235H', name: 'S235 + keményedés (H′)', E: 21000, nu: 0.3, sigmaY: 23.5, alpha: 1.2e-5, rho: 7850, Hp: 420, plastic: true, source: 'kísérleti', verified: false },
  AW6082: { id: 'AW6082', name: 'EN AW-6082 T6 alumínium', E: 7000, nu: 0.33, sigmaY: 25.0, alpha: 2.3e-5, rho: 2700, Hp: 0, plastic: true, source: 'MSZ EN 1999-1-1', verified: false },
  C25: { id: 'C25', name: 'C25/30 beton (csak rugalmas)', E: 3100, nu: 0.2, sigmaY: 0, alpha: 1.0e-5, rho: 2500, Hp: 0, plastic: false, source: 'MSZ EN 1992-1-1 · E a tartós terhekhez (Ebt)', verified: false },
  GL24h: { id: 'GL24h', name: 'GL24h rag. fa (csak rugalmas)', E: 1150, nu: 0.3, sigmaY: 0, alpha: 5e-6, rho: 420, Hp: 0, plastic: false, source: 'MSZ EN 14080 · légszáraz, 12% nedvesség; lassú alakváltozás nincs figyelembe véve', verified: false }
};
...
export function makeLayers(sec, nL) {
  const h = sec.h / 1000, layers = [];
  if (sec.kind === 'rect') {
    const t = h / nL, b = sec.b / 1000;
    for (let i = 0; i < nL; i++) layers.push({ b, t, z: -h / 2 + (i + 0.5) * t });
    return layers;
  }
  if (sec.kind === 'circle' || sec.kind === 'tube') {
    const ro = sec.D / 2000, ri = sec.kind === 'tube' ? ro - sec.t / 1000 : 0;
    const t = 2 * ro / nL;
    for (let i = 0; i < nL; i++) {
      const z = -ro + (i + 0.5) * t;
  ...
  const tf = sec.tf / 1000, tw = sec.tw / 1000, bf = sec.b / 1000, hw = h - 2 * tf;
  let nf = Math.max(2, Math.round(nL * tf / h));
  let nw = Math.max(4, nL - 2 * nf);
  const regions = [[-h / 2, tf, bf, nf], [-h / 2 + tf, hw, tw, nw], [h / 2 - tf, tf, bf, nf]];
...
export function sectionProps(sec, mat, nL, layerOwnInertia) {
  ...
  const E = mat.E * 1e4, G = E / (2 * (1 + mat.nu)), sY = mat.sigmaY * 1e4;
  let A = 0, I = 0, Mp = 0;
  ...
  const kappaS = sec.kind === 'rect' ? 5 / 6
    : sec.kind === 'circle' ? 0.9
      : sec.kind === 'tube' ? 0.5
        : (sec.tw / 1000 * h) / A; // I/U-szelvény: gerinc nyírt területe
  return {
    layers, A, I, Mp, Me, c: Me > 0 ? Mp / Me : 0, h,
    EI: E * I, GAs: kappaS * G * A, E, G, sY, kappaS,
    Hp: mat.Hp * 1e4, rho: mat.rho, plastic: mat.plastic !== false,
    Acat: sec.Acat, Icat: sec.Icat
  };
}
...
function nodeAt(mesh, xt) {
  ...
  for (let i = 0; i < mesh.x.length; i++) { const d = Math.abs(mesh.x[i] - xt); if (d < bd) { bd = d; best = i; } }
  return best;
}
...
export const PRESETS = {
  ...
  simple: { id: 'simple', name: 'Kéttámaszú, egyenletes q', supports: [{ r: 0, type: 'pinned' }, { r: 1, type: 'roller' }], load: { q: 20, P: 0, xP: 0.5 }, ref: 'V-02 / P-05' },
  simpleP: { id: 'simpleP', name: 'Kéttámaszú, középen P', supports: [{ r: 0, type: 'pinned' }, { r: 1, type: 'roller' }], load: { q: 0, P: 60, xP: 0.5 }, ref: 'P-04' },
  clamped: { id: 'clamped', name: 'Kétoldalt befogott, q', supports: [{ r: 0, type: 'fixed' }, { r: 1, type: 'fixed' }], load: { q: 40, P: 0, xP: 0.5 }, ref: 'P-06' },
  twospan: { id: 'twospan', name: 'Kétnyílású folytatólagos, q', supports: [{ r: 0, type: 'pinned' }, { r: 0.5, type: 'roller' }, { r: 1, type: 'roller' }], load: { q: 30, P: 0, xP: 0.25 }, ref: 'P-08' }
};
...
function loadVector(mesh, spec, props) {
  const f = new Float64Array(mesh.nDof), J = mesh.Le / 2;
  let q = spec.load.q || 0;
...
function elemK(EI, GAs, Le, mode) {
  const K = new Float64Array(36), J = Le / 2;
  for (const [xi, wt] of G3) {
    const { dN } = shp(xi), B = [0, dN[0] / J, 0, dN[1] / J, 0, dN[2] / J], c = wt * J * EI;
    for (let i = 0; i < 6; i++) for (let j = 0; j < 6; j++) K[i * 6 + j] += c * B[i] * B[j];
  }
  for (const [xi, wt] of (mode === 'full' ? G3 : G2)) {
    const { N, dN } = shp(xi), B = [-dN[0] / J, N[0], -dN[1] / J, N[1], -dN[2] / J, N[2]], c = wt * J * GAs;
    for (let i = 0; i < 6; i++) for (let j = 0; j < 6; j++) K[i * 6 + j] += c * B[i] * B[j];
  }
  return K;
}

function dofsOf(el) { return [2 * el[0], 2 * el[0] + 1, 2 * el[1], 2 * el[1] + 1, 2 * el[2], 2 * el[2] + 1]; }

function solveDense(K, f, n, fixed) {
  const free = []; for (let i = 0; i < n; i++) if (!fixed.has(i)) free.push(i);
  const m = free.length, A = new Float64Array(m * m), b = new Float64Array(m);
  for (let i = 0; i < m; i++) { b[i] = f[free[i]]; for (let j = 0; j < m; j++) A[i * m + j] = K[free[i] * n + free[j]]; }
  // Gauss-elimináció részleges főelemkereséssel
  for (let k = 0; k < m; k++) {
    let p = k, mx = Math.abs(A[k * m + k]);
    for (let i = k + 1; i < m; i++) { const v = Math.abs(A[i * m + k]); if (v > mx) { mx = v; p = i; } }
    if (mx < 1e-12) return { u: null, singular: true, freeDof: free[k] };
    if (p !== k) { for (let j = 0; j < m; j++) { const t = A[k * m + j]; A[k * m + j] = A[p * m + j]; A[p * m + j] = t; } const t = b[k]; b[k] = b[p]; b[p] = t; }
    for (let i = k + 1; i < m; i++) {
      const fct = A[i * m + k] / A[k * m + k]; if (fct === 0) continue;
      for (let j = k; j < m; j++) A[i * m + j] -= fct * A[k * m + j];
      b[i] -= fct * b[k];
    }
  }
  const y = new Float64Array(m);
  for (let i = m - 1; i >= 0; i--) { let s = b[i]; for (let j = i + 1; j < m; j++) s -= A[i * m + j] * y[j]; y[i] = s / A[i * m + i]; }
...
function assembleK(mesh, props, mode, EItan) {
  const n = mesh.nDof, K = new Float64Array(n * n);
...
function elemKTan(EIgp, GAs, Le, mode) {
  const K = new Float64Array(36), J = Le / 2;
  G3.forEach(([xi, wt], g) => {
    const { dN } = shp(xi), B = [0, dN[0] / J, 0, dN[1] / J, 0, dN[2] / J], c = wt * J * EIgp[g];
    for (let i = 0; i < 6; i++) for (let j = 0; j < 6; j++) K[i * 6 + j] += c * B[i] * B[j];
  });
  for (const [xi, wt] of (mode === 'full' ? G3 : G2)) {
    const { N, dN } = shp(xi), B = [-dN[0] / J, N[0], -dN[1] / J, N[1], -dN[2] / J, N[2]], c = wt * J * GAs;
...
function strainsAt(u, el, xi, Le) {
  const J = Le / 2, { N, dN } = shp(xi), d = dofsOf(el);
  let kap = 0, gam = 0;
  for (let i = 0; i < 3; i++) {
...
function sampleFields(mesh, u, Mgp, Tgp, nSub, Mcap) {
  ...
  mesh.els.forEach((el, e) => {
    const xc = (mesh.x[el[0]] + mesh.x[el[2]]) / 2, d = dofsOf(el);
    for (let s = 0; s <= nSub; s++) {
      const xi = -1 + 2 * s / nSub, { N } = shp(xi);
      let w = 0, ph = 0;
      for (let i = 0; i < 3; i++) { w += N[i] * u[d[2 * i]]; ph += N[i] * u[d[2 * i + 1]]; }
      out.x.push(xc + xi * mesh.Le / 2); out.w.push(w); out.phi.push(ph);
  ...
  for (let e = 1; e < mesh.els.length; e++) {
    const iL = e * (nSub + 1) - 1, iR = e * (nSub + 1);
...
export function solveLinear(spec, props) {
  ...
  for (let i = 0; i < mesh.nDof; i++) { let s = 0; for (let j = 0; j < mesh.nDof; j++) s += K[i * mesh.nDof + j] * u[j]; Ku[i] = s; }
  const reactions = [];
  for (const s of spec.supports) {
    const n = nodeAt(mesh, s.r * spec.L);
  ...
  return {
    ok: true, mesh, u, f, fields, reactions, Mgp, Tgp,
...
function updateGP(st, kap, props, trial) {
  const L = props.layers, E = props.E, H = props.Hp, sY = props.sY;
  const dkap = kap - st.kap;
  let M = 0, EIt = 0, nYield = 0;
  for (let l = 0; l < L.length; l++) {
    const z = L[l].z, dEps = z * dkap;
    const s0 = st.sig[l], a0 = st.alpha[l];
    const sTr = s0 + E * dEps;
    const fy = Math.abs(sTr) - (sY + H * a0);
    let s = sTr, a = a0, ep = st.epsP[l], Et = E, yl = 0;
    if (fy > 0) {
      const dg = fy / (E + H), sg = Math.sign(sTr);
      s = sTr - E * dg * sg; a = a0 + dg; ep = st.epsP[l] + dg * sg;
      Et = E * H / (E + H); yl = 1;
    }
    trial.sig[l] = s; trial.alpha[l] = a; trial.epsP[l] = ep; trial.yl[l] = yl;
    if (yl) nYield++;
    M += s * L[l].b * z * L[l].t;
...
export function solveNonlinear(spec, props, opts) {
  ...
  const mesh = buildMesh(spec), n = mesh.nDof, nL = props.layers.length;
  const fRef = loadVector(mesh, spec, props);
  const fixed = fixedDofs(mesh, spec);
  const nEl = mesh.els.length;

  const gp = [], trialBuf = { sig: new Float64Array(nL), alpha: new Float64Array(nL), epsP: new Float64Array(nL), yl: new Uint8Array(nL) };
  for (let e = 0; e < nEl; e++) { gp.push([newGPState(nL), newGPState(nL), newGPState(nL)]); }

  let u = new Float64Array(n), lam = 0, dlam = o.dLambda;
  const history = [], convLog = [];
  let status = 'ok', steps = 0, totalIters = 0;

  const monitorDof = (() => {
    let best = 0, bv = 0;
    const lin = solveDense(assembleK(mesh, props, spec.integration, null), fRef, n, fixed);
    if (lin.u) for (let i = 0; i < n; i += 2) if (Math.abs(lin.u[i]) > bv) { bv = Math.abs(lin.u[i]); best = i; }
    return best;
  })();

  history.push({ lam: 0, disp: 0, u: new Float64Array(n), M: new Array(nEl).fill(0).map(() => [0, 0, 0]), T: new Array(nEl).fill(0).map(() => [0, 0]), yieldFrac: new Array(nEl).fill(0), yieldGP: new Array(nEl * 3).fill(0), sig: new Float32Array(nEl * 3 * nL), iters: 0, res: 0 });

  const J = mesh.Le / 2;
  const evalState = (uu, commit) => {
    const p = new Float64Array(n), Mgp = [], Tgp = [], EIt = [], yieldGP = [];
    const sigSnap = commit ? new Float32Array(nEl * 3 * nL) : null;
    mesh.els.forEach((el, e) => {
      const d = dofsOf(el), Ms = [0, 0, 0], Ts = [0, 0], Es = [0, 0, 0], pe = new Float64Array(6);
      G3.forEach(([xi, wt], g) => {
        const { kap } = strainsAt(uu, el, xi, mesh.Le);
        const r = updateGP(gp[e][g], kap, props, trialBuf);
        Ms[g] = r.M; Es[g] = r.EIt; yieldGP.push(r.nYield / nL);
        if (commit) {
          gp[e][g].sig.set(trialBuf.sig); gp[e][g].alpha.set(trialBuf.alpha);
          gp[e][g].epsP.set(trialBuf.epsP); gp[e][g].kap = kap;
          for (let l = 0; l < nL; l++) sigSnap[(e * 3 + g) * nL + l] = trialBuf.sig[l];
        }
        const { dN } = shp(xi), B = [0, dN[0] / J, 0, dN[1] / J, 0, dN[2] / J];
        for (let i = 0; i < 6; i++) pe[i] += wt * J * B[i] * r.M;
      });
      G2.forEach(([xi, wt], g) => {
        const { gam } = strainsAt(uu, el, xi, mesh.Le);
        Ts[g] = props.GAs * gam;
        const { N, dN } = shp(xi), B = [-dN[0] / J, N[0], -dN[1] / J, N[1], -dN[2] / J, N[2]];
        for (let i = 0; i < 6; i++) pe[i] += wt * J * B[i] * Ts[g];
      });
      for (let i = 0; i < 6; i++) p[d[i]] += pe[i];
      Mgp.push(Ms); Tgp.push(Ts); EIt.push(Es);
    });
  ...
  const restoreState = (S) => { for (let e = 0; e < nEl; e++) for (let g = 0; g < 3; g++) { gp[e][g].sig.set(S[e][g].sig); gp[e][g].alpha.set(S[e][g].alpha); gp[e][g].epsP.set(S[e][g].epsP); gp[e][g].kap = S[e][g].kap; } };
  ...
  while (seg < path.length && guard++ < 1200) {
    ...
    if (Math.abs(target - lam) < 1e-9) { seg++; dlam = o.dLambda; continue; }
    const dir = target > lam ? 1 : -1;
    const uSave = snapshot(u), stSave = saveState(), lamSave = lam;
    let trial = lam + dir * dlam;
    if (dir > 0 ? trial > target : trial < target) trial = target;
    const f = new Float64Array(n); for (let i = 0; i < n; i++) f[i] = fRef[i] * trial;

    let cur = evalState(u, false);
    let converged = false, it = 0, stepLog = [];
    let KT = null;
    while (it < o.iterMax) {
      const psi = new Float64Array(n); for (let i = 0; i < n; i++) psi[i] = f[i] - cur.p[i];
      let nps = 0, nf = 0;
      for (let i = 0; i < n; i++) if (!fixed.has(i)) { nps += psi[i] * psi[i]; nf += f[i] * f[i]; }
      const res = nf > 1e-12 ? 100 * Math.sqrt(nps) / Math.sqrt(nf) : (Math.sqrt(nps) < 1e-8 ? 0 : 100);
      if (it > 0) stepLog.push(res);
      if (it > 0 && res <= o.tol) { converged = true; break; }
      if (o.algo === 'newton' || KT === null) KT = assembleK(mesh, props, spec.integration, cur.EIt);
      const sol = solveDense(KT, psi, n, fixed);
      if (!sol.u) { converged = false; break; }
      for (let i = 0; i < n; i++) u[i] += sol.u[i];
      cur = evalState(u, false);
      it++; totalIters++;
    }
    if (!converged) {
      u = uSave; restoreState(stSave); lam = lamSave;
      dlam = dlam / 2;
      if (dlam < o.minStep) { status = steps === 0 ? 'no-progress' : 'limit-load-reached'; break; }
    ...
    history.push({
      lam, disp: u[monitorDof], u: snapshot(u), M: fin.Mgp, T: fin.Tgp,
      yieldFrac, yieldGP: fin.yieldGP.slice(), sig: fin.sigSnap, iters: it, res: stepLog[stepLog.length - 1] || 0
    });
    if (it < o.iterMin) dlam = Math.min(dlam * 1.5, o.dLambda * 2);
    if (Math.abs(lam - target) < 1e-9) { seg++; dlam = o.dLambda; }
  }

  const nSub = 6;
  const framesFields = history.map(h => sampleFields(mesh, h.u, h.M, h.T, nSub, props.Mp * (props.Hp > 0 ? 1.4 : 1)));
  return {
    ok: true, mesh, history, framesFields, convLog, status, steps, totalIters,
    lambdaU: history.reduce((a, h) => Math.max(a, h.lam), 0), lambdaEnd: history[history.length - 1].lam,
    monitorDof, nL, fRef, path,
...
export function residualCheck(nl, props, frame) {
  const h = nl.history[Math.min(frame, nl.history.length - 1)], nL = props.layers.length;
  const nEl = nl.mesh.els.length;
  let resN = 0, resM = 0, normN = 0, normM = 0, sMax = 0;
  for (let e = 0; e < nEl; e++) for (let g = 0; g < 3; g++) {
    let sN = 0, sM = 0, aN = 0, aM = 0;
    for (let l = 0; l < nL; l++) {
      const s = h.sig[(e * 3 + g) * nL + l], A = props.layers[l].b * props.layers[l].t;
      sN += s * A; sM += s * A * props.layers[l].z;
      aN += Math.abs(s) * A; aM += Math.abs(s) * A * Math.abs(props.layers[l].z);
      if (Math.abs(s) > sMax) sMax = Math.abs(s);
    }
    resN = Math.max(resN, Math.abs(sN)); resM = Math.max(resM, Math.abs(sM));
    normN = Math.max(normN, aN); normM = Math.max(normM, aM);
  }
  return {
...
export function analytic(spec, props) {
  const { L } = spec, EI = props.EI, GAs = props.GAs, q = spec.load.q, P = spec.load.P, Mp = props.Mp;
  const s = spec.presetId;
  if (s === 'cantilever') return { w: P * L ** 3 / (3 * EI) + P * L / GAs, label: 'w = PL³/3EI + PL/GAs', Pu: Mp / L, uLabel: 'Pᵤ = Mp/L' };
  if (s === 'simple') return { w: 5 * q * L ** 4 / (384 * EI) + q * L * L / (8 * GAs), label: 'w = 5qL⁴/384EI + qL²/8GAs', Pu: 8 * Mp / (L * L), uLabel: 'qᵤ = 8Mp/L²' };
  if (s === 'simpleP') return { w: P * L ** 3 / (48 * EI) + P * L / (4 * GAs), label: 'w = PL³/48EI + PL/4GAs', Pu: 4 * Mp / L, uLabel: 'Pᵤ = 4Mp/L' };
  if (s === 'clamped') return { w: q * L ** 4 / (384 * EI) + q * L * L / (8 * GAs), label: 'w = qL⁴/384EI', Pu: 16 * Mp / (L * L), uLabel: 'qᵤ = 16Mp/L²' };
