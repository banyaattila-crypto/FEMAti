...
(() => {
  ...
  function encodeCamelAttrs(html) {
    return html.replace(CAMEL_ATTR_RE, (_, sp, name, eq) => sp + CAMEL_ATTR + name.replace(/[A-Z]/g, (c) => '-' + c.toLowerCase()) + eq);
  }
  ...
  function walkXImport(el, host) {
    ...
    const fromRaw =
      el.getAttribute('from') || (el.getAttribute('component-from-global-scope') ? '' : el.getAttribute('src') || el.getAttribute('import') || '');
    ...
    class StreamableComponent extends React.Component {
      ...
      __reconcileLogic() {
        ...
        if (
          ...
          (Next === this.__failedLogic && r.ver === this.__failedVer && shallowEqual(this.__userProps(), this.__failedUserProps))
        ) {
          return;
        }
  ...
  function createHelmetManager(doc, isStreaming) {
    ...
    try {
      ...
      appTheme =
        ds === 'dark' || ds === 'light' ? ds : new URLSearchParams(doc.defaultView?.location.search ?? '').get('theme') === 'dark' ? 'dark' : 'light';
    } catch {}
