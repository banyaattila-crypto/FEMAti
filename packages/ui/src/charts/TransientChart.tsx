...
export function TransientChart({ run, quantity, lang = 'hu' }: TransientChartProps): JSX.Element {
  const t = CHARTS[lang];
  ...
  const zeroY = sy(0);
  const label = quantity === 'w' ? REPORT[lang].diagramTitle.w : t.labelAcceleration;
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={t.transientAriaLabel(label, referenceX.toFixed(2), `${format(peak.value).value} ${format(peak.value).unit}`)}
