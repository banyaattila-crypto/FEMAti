...
> **KÖTELEZŐ OLVASMÁNY: [docs/HIBATURESI-POLITIKA.md](docs/HIBATURESI-POLITIKA.md).**
> Az alábbi táblázatok „tűrés" oszlopa nem egyetlen mérnöki szám, hanem
> **hibafajta szerinti korlát**:
>
> - **Elvileg egzakt azonosság** (patch test, merevtest-mozgás, szimmetria,
>   egyensúly) → **gépi pontosság** (1e−12…1e−14). Itt „elfogadható eltérés"
>   nem létezik; ami nem egzakt, az kódhiba.
> - **Két független algoritmus egyezése** → 1e−10 relatív.
> - **Diszkretizációs hiba** (hálósűrűség, rétegszám, teherlépcső) → **nem
>   abszolút szám a követelmény, hanem a konvergencia rendjének igazolása.**
>   Egyetlen hálón mért eltérés önmagában nem elfogadási kritérium.
> - **Rosszul kondicionált eset** (karcsú gerenda, penalty-módszer) → a korlát
>   a kondíciószámból származtatva, a tesztben kimondva.
>
> Ahol a táblázat százalékos értéket ad, az **diszkretizációs** hibára
> vonatkozik, és mindig konvergencia-vizsgálattal együtt értendő.
...
> **Kötelező megkötés az [ADR-0005](docs/ADR/0005-levezetes-ujraszamolassal.md) miatt:**
> minden részlépés önállóan, mellékhatásmentesen hívható publikus függvény kell
> legyen (`shapeFunctions`, `jacobian`, `bMatrix`, `constitutiveMatrix`,
> `gaussRule`, `elementStiffness`). A levezetés-modul ezeket hívja lépésenként —
> nem másolja a képleteket, hanem ugyanazt a kódot futtatja, hogy a
> jegyzőkönyv és a megoldó garantáltan ugyanazt a számot adja.
...
- linearSolver.ts: solveLinear(model) → LinearResult.

A LinearResult alakját a DESIGN-TERV.md 8. fejezete rögzíti (a felület ↔ mag
adatszerződés). Kötelező mezők már ebben a fázisban:
  displacements, reactions, extremes, props (EI, GAs, A, I, Me, Mp, c),
  dofCount, equilibrium { sumFz, sumMy }, errorEstimate.
Ezek nélkül a P7 felület nem építhető meg — a jobb oldali eredménypanel és az
egyensúly-ellenőrzés pontosan ezekre épül.
...
> **Kötelező bemenet: [DESIGN-TERV.md](DESIGN-TERV.md).** A tokenek, az elrendezés,
> a rajzjelek és a számformázás ott vannak rögzítve, a `design/` mappa három
> canvas-variánsa alapján. A felület nem tervezhető újra ebben a fázisban.

```
Építsd meg az ui csomag alapját: React + Zustand + SVG modellező vászon.
KÖVESD a DESIGN-TERV.md 2., 3., 4.1–4.2 és 5.1–5.3 fejezetét pontosan.
...
- Konvergencia PONTOSAN: 100·√(Σψi²)/√(Σfi²) ≤ tolerancia.
  ALAPÉRTELMEZÉS: 1e-4 % (= 1e-6 relatív), NEM a diplomaterv 0.5%-a.
  A 0.5% 1996-os gépidő-korlát volt; mai gépen indokolatlanul laza, mert a
  maradó kiegyensúlyozatlan erő a képlékeny csuklók helyét is elmozdíthatja.
  A 0.5% külön kapcsolóval elérhető marad ("történelmi mód").
  Lásd docs/HIBATURESI-POLITIKA.md 5. pont.
...
### P15/A — Levezetés: teljes számítás bemutatása, DOCX és PDF export

```
Implementáld a `derivation` modult a fem-core-ban és a hozzá tartozó
"Levezetés" nézetet az ui-ban. Kötelező olvasmány: ADR-0005.

A modul NEM másolja a képleteket: a mag publikus, finom szemcsézetű
függvényeit hívja lépésenként, és feljegyzi a köztes értékeket.

A levezetés szerkezete (a megrendelő által választott mélység):

1. FELADAT
   - statikai váz, geometria, peremfeltételek, terhek — táblázatosan
   - anyagjellemzők forrásmegjelöléssel (verified jelzéssel)

2. KERESZTMETSZET
   - a rétegzés levezetése: bl, tl, zl táblázat
   - A = Σ bl·tl és I = Σ bl·zl²·tl behelyettesített számokkal (3.54)
   - Me = σY·I/ymax, Mp = σY·2S0, c = Mp/Me (3.37, 3.39, 3.40)
   - összevetés a szelvénytáblázat adatával, az eltérés kiírva

3. VÉGESELEM-FELOSZTÁS
   - elemszám, elemhossz, csomópont-koordináták
   - szabadságfokok kiosztása, a globális DOF-térkép

4. EGY VÁLASZTOTT ELEM TELJES LEVEZETÉSE   ← a lényeg
   - a Gauss-pontok helye és súlya (3 pontos és 2 pontos szabály)
   - N1..N3 és dNi/dξ értéke MINDEN Gauss-pontban, számokkal
   - Jacobi: J = dx/dξ, |J|, J⁻¹ (3.7–3.9)
   - a B mátrix 2×6 alakja minden Gauss-pontban, kitöltve (3.14)
   - a D anyagmátrix: EI és GAs számítása
   - a Kₑ integrálás lépései: a hajlítási tag 3 pontból, a nyírási 2 pontból
   - a 6×6 Kₑ mátrix VÉGSŐ alakja számokkal
   - az elemi tehervektor integrálása (3.19–3.23)

5. KOMPILÁLÁS ÉS MEGOLDÁS
   - a globális mátrix mérete, profilja, frontszélessége
   - a peremfeltételek beépítése
   - a megoldás módszere, a szinguláris pivot ellenőrzése

6. EREDMÉNYEK ÉS ELLENŐRZÉSEK
   - csomóponti elmozdulások táblázata
   - igénybevételek, a Gauss-pontból csomópontba extrapolálás lépése
   - egyensúly-ellenőrzés: ΣFz, ΣMy
   - analitikus összevetés, ahol létezik, az eltérés %-ban

7. KÉPLÉKENY SZÁMÍTÁS (ha futott)
   - teherlépcsőnkénti napló: λ, iterációszám, reziduum
   - EGY választott Gauss-pont rétegenkénti feszültségszámítása:
     a próbafeszültség, az R redukciós tényező, a visszavetítés (3.4.4)
   - a képlékeny csuklók kialakulási sorrendje
   - határteher összevetése a képlékeny elmélettel

KÖTELEZŐ TESZT (elfogadási kritérium):
A levezetés által számított Kₑ, tehervektor és Gauss-ponti feszültség
BIT-AZONOS a solver által használt értékekkel. Nem "1e-12-n belül" —
azonos, mert ugyanaz a függvény számolja.

EXPORT:
- .docx: valódi OOXML a `docx` könyvtárral; képletek strukturált szövegként
  (monospace, Unicode jelekkel), nem képként — szerkeszthető és kereshető
- .pdf: a nyomtatási nézetből, hogy a PDF és a képernyő tartalma azonos legyen
- a fejlécben: a modell neve, a dátum, a szoftver verziója és a mag commit-hash-e
```
